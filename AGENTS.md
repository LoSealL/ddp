...
| `DDP_SHM_SIZE` | `8Gi` | 业务 pod `/dev/shm` emptyDir(Memory) 上限 |
...
- **共享内存与宿主机映射**：debug pod 和 GPU Job 都挂 8Gi `/dev/shm`（tmpfs emptyDir）。作业可声明 `node_name`（选定节点，PVC 已绑定他节点时提交报错）和 `host_mounts`（JSON `[{host, container}]`，hostPath type=Directory，宿主机系统目录黑名单见 `main._HOST_BLACKLIST`，容器侧禁覆盖 /workspace、/dev/shm 及系统路径）。仅创建时可指定，PATCH 编辑不支持。
