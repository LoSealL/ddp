...
IMAGE_REPOS = [r.strip() for r in os.environ.get(
    "DDP_IMAGE_REPOS", "ddp-cuda-ssh,ddp-pytorch-ssh").split(",") if r.strip()]
...
def list_images() -> list[str]:
    ...
    for repo in IMAGE_REPOS:
        ...
        tags = sorted({t["name"] for a in artifacts for t in (a.get("tags") or [])},
                      reverse=True)
        tags.sort(key=lambda t: t != "latest")  # latest first
